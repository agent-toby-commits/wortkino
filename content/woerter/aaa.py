#!/usr/bin/env python3
# textredigierung
"""Verbessert alle Markdown-Dateien im Ordner dieses Skripts."""
...
def hauptprogramm() -> None:
    ...
    ausgeschlossene_dateien = {pfad.resolve() for pfad in beispielpfade}
    quelldateien = sorted(
        (
            pfad
            for pfad in skriptordner.glob("*.md")
            if not pfad.stem.casefold().endswith("-neu")
            and pfad.resolve() not in ausgeschlossene_dateien
        ),
